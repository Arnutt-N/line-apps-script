...
function doGet(e) {
  ...
  if (String(params.mode || '').toLowerCase() === 'health') {
    return jsonOutput_({
      ok: true,
      app: 'gas-main',
      ...
      ownerEmailConfigured: !!getProperty_('ADMIN_OWNER_EMAIL', ''),
      sheetNames: {
  ...
  // Default: render the Phase 1 admin panel.
  return renderAdminPanel_(e || {});
}
